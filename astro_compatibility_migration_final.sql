-- Astrological Compatibility & Ice Breakers Migration - FINAL VERSION
...
-- Step 12: Ensure all profiles have gender data (if not already set)
UPDATE public.profiles 
SET gender = CASE 
  WHEN gender IS NULL AND name = 'Ashley' THEN 'female'
  WHEN gender IS NULL AND name IN ('RESHAB', 'Daniel') THEN 'male'
  WHEN gender IS NULL AND name IN ('Sophia', 'Ava') THEN 'female'
