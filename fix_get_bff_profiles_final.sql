-- Fix get_bff_profiles function - Use only columns that exist
-- Removed: interests (doesn't exist), bio (doesn't exist)
-- Fixed: BFF check to use bff_enabled column properly

-- 1. Drop the existing function
DROP FUNCTION IF EXISTS public.get_bff_profiles(UUID);

-- 2. Create the function with only columns that exist
...
BEGIN
  ...
  WHERE p.id != p_user_id
    -- Check bff_enabled column (this is what exists and is set to true)
