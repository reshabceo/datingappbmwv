...
class SimpleCameraScreen extends StatefulWidget {
  ...
  const SimpleCameraScreen({super.key, required this.matchId});
...
class _SimpleCameraScreenState extends State<SimpleCameraScreen> {
  ...
  Future<void> _takePhoto() async {
    try {
      if (_cameraController != null && _cameraController!.value.isInitialized) {
        ...
        print('🔍 DEBUG: Camera not ready, using image picker fallback...');
        final XFile? photo = await _picker.pickImage(
          source: ImageSource.camera,
          maxWidth: 1080, // Reduce size for faster processing
          maxHeight: 1080,
  ...
  Future<void> _selectFromGallery() async {
    try {
      print('🔍 DEBUG: Selecting from gallery...');
