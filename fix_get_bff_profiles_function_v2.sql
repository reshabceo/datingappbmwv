-- Fix get_bff_profiles function with correct column names
-- This version uses only the columns that actually exist in the profiles table

-- 1. Drop the existing function
DROP FUNCTION IF EXISTS public.get_bff_profiles(UUID);

-- 2. Create a fixed version with only existing columns
...
DO $$
BEGIN
  RAISE NOTICE '=== get_bff_profiles Function Fixed (v2) ===';
