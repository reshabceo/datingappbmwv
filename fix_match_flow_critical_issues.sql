-- Fix Critical Match Flow Issues
-- This script addresses all the critical problems identified in the match flow analysis

-- Step 1: Fix the matches table structure to match Flutter expectations
...
-- Step 2: Create the swipes table (if it doesn't exist)
...
-- Step 3: Add gender column to profiles table
ALTER TABLE public.profiles 
ADD COLUMN IF NOT EXISTS gender TEXT,
ADD COLUMN IF NOT EXISTS last_seen TIMESTAMP WITH TIME ZONE DEFAULT NOW();

-- Step 4: Create user preferences table for filtering
...
-- Step 6: Create RLS policies for swipes
...
-- Step 7: Create RLS policies for user preferences
CREATE POLICY "Users can manage their own preferences"
ON public.user_preferences FOR ALL
USING (auth.uid() = user_id);

-- Step 8: Fix the handle_swipe function with proper self-match prevention
...
-- Step 9: Create function to get filtered profiles with gender preferences
...
BEGIN
  ...
  SELECT 
    ...
    up.max_distance,
    p.gender
  INTO v_user_preferences, v_user_gender
  ...
  WHERE pr.id != p_user_id
    ...
    AND (v_user_preferences.preferred_gender IS NULL 
         OR pr.gender = ANY(v_user_preferences.preferred_gender)
         OR pr.gender IS NULL) -- Include profiles without gender if no preference set
    AND (v_user_preferences.min_age IS NULL OR pr.age >= v_user_preferences.min_age)
    AND (v_user_preferences.max_age IS NULL OR pr.age <= v_user_preferences.max_age)
    AND pr.id NOT IN (
...
-- Step 10: Clean up existing self-matches
DELETE FROM public.matches 
WHERE user_id_1 = user_id_2;

-- Step 11: Update existing profiles with sample gender data
...
-- Step 13: Create function to set user preferences
...
-- Step 15: Test the handle_swipe function
-- This will be tested when the Flutter app calls it
