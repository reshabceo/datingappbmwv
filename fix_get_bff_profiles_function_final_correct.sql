-- FINAL CORRECT FIX: Create get_bff_profiles function with correct data types
-- Fix the data type mismatch and remove super likes from BFF mode

-- 1. Drop the existing function
DROP FUNCTION IF EXISTS public.get_bff_profiles(UUID);

-- 2. Create the function with correct data types
...
DO $$
BEGIN
  RAISE NOTICE '=== get_bff_profiles Function Fixed (FINAL CORRECT) ===';
  RAISE NOTICE 'Fixed data type mismatch (photos and hobbies are JSONB)';
  RAISE NOTICE 'BFF mode does NOT use super likes - only regular likes/passes';
